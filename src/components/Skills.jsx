  ...
  return (
    <div className='p-3 mt-10 mb-6 w-full overflow-x-hidden min-h-screen'>
        ...
        <div className=' mt-10 flex max-md:flex-col justify-evenly items-center'>
            <diV className={`my-5 w-72 md:w-[20rem] bg-white dark:text-white dark:bg-slate-950/60 shadow-sm dark:shadow-slate-500 flex flex-wrap justify-evenly py-4 rounded-xl ${skills?"animate":null}`}>
                {skillList1.map((skill,index)=>(
                    <div key={index} className=' flex p-2 my-3'>
                    <div className=' mt-2'><Tick/></div>
                    <div className=' ml-2'>
                        <h4 className=' font-semibold'>{skill.name}</h4>
                        <p className=' text-sm dark:text-slate-400'>{skill.level}</p>
                    </div>
                </div>
                ))}
            </diV>
